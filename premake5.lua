...
project "hood-detach"
	...
	filter { "configurations:*III" }
		...
		targetname "HoodDetachIII"
		debugdir "$(GTA_III_DIR)"
		debugcommand "$(GTA_III_DIR)/gta3.exe"
		postbuildcommands "copy /y \"$(TargetPath)\" \"$(GTA_III_DIR)\\scripts\\HoodDetachIII.asi\""
	...
	filter { "configurations:*VC" }	
		...
		targetname "HoodDetachVC"
		debugdir "$(GTA_VC_DIR)"
		debugcommand "$(GTA_VC_DIR)/gta-vc.exe"
		postbuildcommands "copy /y \"$(TargetPath)\" \"$(GTA_VC_DIR)\\scripts\\HoodDetachVC.asi\""
